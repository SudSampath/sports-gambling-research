...
- **Scoring-luck decomposition** (`scoring_luck.py`, `scoring_luck_evaluation.py`): red-zone touchdown-rate, special-teams EPA, and turnover-margin differentials, each independently fit and ablated as additive logit adjustments. **Rejected as the default**: red-zone and special-teams are flat-to-negligible on all three test seasons, and the turnover coefficient is unstable across folds -- it saturated at the search bound on the 2015-2022 training fold (quasi-separation, not a scale bug) and produced catastrophic 2023 test-fold miscalibration. See the [2026-08-27 research note](docs/research/scoring-luck-2026-08-27.md) for the full comparison table, the instability writeup, and the `evaluate-scoring-luck` CLI command.
